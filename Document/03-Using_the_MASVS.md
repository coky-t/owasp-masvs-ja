...
## モバイルアプリケーションセキュリティプロファイル

MAS プロジェクトは従来三つの検証レベル (L1, L2, R) を提供してきましたが、2023 年の MASVS リファクタリングで見直し、「セキュリティテストプロファイル」として作り直し、OWASP MASTG に移行されました。これらのプロファイルは情報システムを保護するために使用できるセキュリティコントロールの包括的なカタログである [NIST OSCAL (Open Security Controls Assessment Language)](https://pages.nist.gov/OSCAL/) 標準と整合するようになりました。
