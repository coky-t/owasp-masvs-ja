...
MAS プロジェクトは従来、ダウンロード可能なアプリに焦点を当ててきたため、そのリソースはすべて「アプリ」に言及しています。しかし、このプロジェクトはプリロードされるアプリケーションや SDK などビジネスの他の分野にも適用できます。これらのカテゴリにはいずれも考慮すべきことがいくつかあります。

**プリロード:** プリロードされるアプリは工場出荷時にユーザーのデバイスにインストールされるアプリで、悪用されやすいビジネス慣行に対してユーザーが脆弱なまま、昇格した権限を持つ可能性があります。平均的なユーザーのデバイスには多数のプリロードされるアプリがあるため、そのリスクを定量化できる方法で測定することが重要です。

デバイスには何百ものプリロードが搭載されている可能性があり、その結果、自動化が重要になります。自動化に適した MAS 基準のサブセットがよい基盤となるかもしれません。

**SDK:** SDK はモバイルアプリのバリューチェーンで重要な役割を果たし、開発者がより速く、よりスマートに、より収益性の高いものを構築するために必要なコードを提供します。開発者は SDK に大きく依存しており、平均的なモバイルアプリで 30 の SDK を使用し、コードの 90% をサードパーティから調達しています。このような広範な使用は開発者に多大な利益をもたらす一方で、安全性とセキュリティの問題も広げてしまいます。

SDK はさまざまな機能を提供するため、すべての SDK タイプにすべての MASVS カテゴリを適用するわけではありません。そのため、SDK 開発者はラボが適切に評価できるように、それぞれの SDK にどのカテゴリを適用するかを評価する必要があります。
...
![Verification Levels](images/masvs-levels-new_ja.jpg)
